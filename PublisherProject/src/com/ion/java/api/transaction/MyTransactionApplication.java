package com.ion.java.api.transaction;
 ...
 */
public class MyTransactionApplication implements MkvPlatformListener, MkvPublishListener, MkvTransactionListener {
    ...
    public void onMain(MkvPlatformEvent mkvPlatformEvent) {
        switch (mkvPlatformEvent.intValue()) {
            case MkvPlatformEvent.START_code:
                ...
                mkv.getPublishManager().addTransactionListener(this);
                publishType();
                publishRecord();
                publishSupplyChain();
                publishPattern();
    ...
    public static void main(String[] args) {
        ...
        qos.setPlatformListeners(new MkvPlatformListener[]{new MyTransactionApplication()});
        try {
...
//        String SOURCE = mkvProperties.getProperty("source");
    ...
    public void onCall(MkvTransactionCallEvent mkvTransactionCallEvent) {
        MkvRecord rec = mkvTransactionCallEvent.getRecord();
        MkvSupply fields = mkvTransactionCallEvent.getSupply();

        try {
            rec.supply(fields);

            try {
                mkvTransactionCallEvent.setResult((byte) 0, "Ok");
            } catch (MkvException e) {
                e.printStackTrace();
            }
        } catch (MkvException e) {
            try {
                mkvTransactionCallEvent.setResult((byte) 1, "Error " +
                        e.getMessage());
            } catch (MkvException e1) {
                e1.printStackTrace();
            }
            e.printStackTrace();
        }
